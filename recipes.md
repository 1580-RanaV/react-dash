...
- **Details / Remix / .md file** tabs (`SubTabCorner`, the same segmented-pill component used for the Steps List/Canvas toggle):
  ...
  - **Remix** doesn't navigate anywhere — clicking it opens Blu Chat and drops in a suggested prompt ("Remix the "X" recipe — keep the structure but adapt it for a different use case"), handing the remix off to the chat rather than a dedicated editor.
  - **.md file** opens a `recipe.md` sliding sidebar rendering the recipe as a markdown-styled export.
- **Run button** (green, top bar) — dispatches `open-blu-chat`, then after a short delay fires the same `blu-recipe-run` window event the pipeline canvas fires when it finishes a run, with `steps: MOCK_STEPS.map(s => s.title)`. This gives every seeded recipe — not just custom canvas-built ones — a working "Run" affordance that produces the same `ExecChecklist` run-card experience in Blu Chat (§6). The button shows its own spinner for the whole duration of that checklist animation (not just until the event fires), so it stays visibly "busy" the entire time steps are ticking through in the chat panel, not just for the brief moment before the event dispatches.
...
- **Remix existing recipe** — clicking the option **immediately** jumps to a searchable recipe picker (no intermediate "click Continue to proceed" step); picking one and clicking **Clone** currently just closes the drawer. **Known gap**: nothing actually clones the recipe or opens the canvas from here — the remix flow's "Clone" action is a dead end in this mock. (The Remix *tab* inside an existing recipe's detail view, §2, is the one remix path that actually does something — it hands off to Blu Chat.)
...
- **Nothing persists.** Reloading the page resets the canvas to empty, resets any card interaction state, and forgets anything built. There's no backend, no localStorage, no draft autosave for a canvas-in-progress.
