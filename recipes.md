...
- **Run history** (`RunRecord[]`, keyed by recipe id) — every time a Run actually completes (i.e., it wasn't locked), a record is appended: a timestamp and a `createdLabel`. This renders as the "Recent runs" list in the detail view (§2). It's explicitly not a real generation/artifact system — `createdLabel` is a plain templated string (`"<recipe> — run output"`), not a reference to anything actually created, and clicking it hands off to Blu Chat rather than opening a real artifact.
